# Plan de déploiement Yeboekun sur Render

Ce document décrit les étapes détaillées pour déployer l'application Yeboekun (backend .NET 9, frontend HTML/JS, MySQL) sur [Render.com](https://render.com).
...
│  yeboekun.onrender.com   │     │  yeboekun-api.onrender.com│    │  Internal URL           │
└─────────────────────────┘     └─────────────────────────┘     └─────────────────────────┘
```
...
- Repo GitHub [yans40/yeboekun](https://github.com/yans40/yeboekun) connecté
...
1. [PlanetScale](https://planetscale.com) → Create database → `yeboekun`
...
```bash
mysql -h <HOST> -P 3306 -u <USER> -p <PASSWORD> yeboekun < scripts/init.sql
```
...
2. Connecter le repo **yans40/yeboekun**
3. **Branch** : `main`
...
Le [backend/Dockerfile](../backend/Dockerfile) utilise des chemins relatifs à `backend/` (`src/Yeboekun.API/`, etc.). Le **Root Directory** doit être `backend` pour que le build fonctionne.
...
| `ConnectionStrings__DefaultConnection` | URL MySQL | `mysql://yeboekun:xxx@host/database` |
...
Modifier [backend/src/Yeboekun.API/Program.cs](../backend/src/Yeboekun.API/Program.cs) pour autoriser l'URL du frontend Render :

```csharp
// En production, ajouter l'URL Render du frontend
policy.WithOrigins(
    "https://yeboekun.onrender.com",
    "https://yeboekun.onrender.com"
)
```
...
window.GEGEDOT_CONFIG = {
  API_BASE_URL: 'https://yeboekun-api.onrender.com/api'
};
```
...
4. Variable d'environnement Render : `API_BASE_URL` = `https://yeboekun-api.onrender.com/api`
...
Ou documenter l'URL directe : `https://yeboekun.onrender.com/professional-fan-view.html`
...
- **Frontend** : `https://yeboekun.onrender.com` (ou `https://yeboekun-xxxx.onrender.com`)
- **Backend API** : `https://yeboekun-api.onrender.com` (ou `https://yeboekun-api-xxxx.onrender.com`)
- **Swagger** : `https://yeboekun-api.onrender.com/swagger` (si activé en prod)
