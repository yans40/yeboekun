...
yeboekun/
├── backend/src/
│   ├── Yeboekun.API/            ← Contrôleurs HTTP, Program.cs
│   ├── Yeboekun.Services/       ← Logique métier, DTOs, AutoMapper
│   ├── Yeboekun.Infrastructure/ ← Repositories, DbContext (EF Core)
│   └── Yeboekun.Core/           ← Entités, interfaces
├── frontend/
│   ├── src/
...
```bash
docker exec -i yeboekun-mysql mysql -uyeboekun -ppassword yeboekun -e "
ALTER TABLE Persons
...
docker logs yeboekun-backend -f

# Accéder à MySQL en ligne de commande
docker exec -it yeboekun-mysql mysql -uyeboekun -ppassword yeboekun

# Recompiler le backend (après modification)
docker restart yeboekun-backend
...
docker exec -i yeboekun-mysql mysql -uyeboekun -ppassword yeboekun < scripts/got_seed.sql

# Charger la famille royale
docker exec -i yeboekun-mysql mysql -uyeboekun -ppassword yeboekun < scripts/royal_family_seed.sql
